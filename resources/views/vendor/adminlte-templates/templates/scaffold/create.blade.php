...
onMounted(() => {
    ...
    <template>
        <Head :title="t('Create {{ $config->modelNames->name }}')" />
    
        <AppLayout :breadcrumbs="breadcrumbs">
            <div class="p-4 sm:p-6 md:p-8 w-full max-w-4xl mx-auto">
                <Card>
                    <CardHeader>
                        <CardTitle>{{ t('Create {{ $config->modelNames->name }}') }}</CardTitle>
                        <CardDescription>{{ t('Fill in the information to register a new {{ $config->modelNames->name }}') }}</CardDescription>
                    </CardHeader>
                    <CardContent>
